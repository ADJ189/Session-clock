...
async function loadYouTubeLibrary(root: HTMLElement): Promise<void> {
  ...
  if (!Integrations.isYouTubeConnected()) {
    ...
    // finish.
    const clientId =
      localStorage.getItem("sc_google_client_id") || DEFAULT_GOOGLE_CLIENT_ID;
    if (!clientId) {
      alert(
        "Set up Google sign-in first: open Settings → Integrations → YouTube (or Google Calendar) — it also needs a Client Secret, which only fits in that form.",
      );
      return;
    }
