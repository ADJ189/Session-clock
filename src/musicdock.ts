...
async function loadYouTubeLibrary(root: HTMLElement): Promise<void> {
  ...
  if (!Integrations.isYouTubeConnected()) {
    // Prefer the site's own registered app (one click, no setup) or a
    // Client ID the visitor already registered via the full self-host
    // form in Settings → Integrations. That form (see
    // googleSelfHostNodes in integrations.ts) also collects the
    // matching Client Secret Google's proxied OAuth now needs, which a
    // single prompt() here can't — so if neither is on file, send the
    // visitor there instead of starting a connection that can never
    // finish.
    const clientId = localStorage.getItem('sc_google_client_id') || DEFAULT_GOOGLE_CLIENT_ID;
    if (!clientId) {
      alert('Set up Google sign-in first: open Settings → Integrations → YouTube (or Google Calendar) — it also needs a Client Secret, which only fits in that form.');
      return;
    }
