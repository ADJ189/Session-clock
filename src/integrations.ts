...
export async function oauthHandleCallback(): Promise<{ provider: string; error?: string } | null> {
  ...
  const clientSecret = cfg.clientSecretKey ? localStorage.getItem(cfg.clientSecretKey) : null;
  ...
  try {
    const res = cfg.direct
      ...
      : await fetch(OAUTH_PROXY, {
          ...
          body: JSON.stringify({
            provider, grant_type: 'authorization_code', code, redirect_uri, code_verifier: verifier,
    ...
    if (!data.access_token) {
      ...
      return { provider, error: data.error_description || data.message || data.error || `sign-in failed (${res.status})` };
    }
    save(provider, {
  ...
  try {
    const res = cfg.direct
      ? await fetch(cfg.tokenUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: new URLSearchParams({ grant_type: 'refresh_token', refresh_token: creds.refresh, client_id: clientId }),
        })
      : await fetch(OAUTH_PROXY, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            provider, grant_type: 'refresh_token', refresh_token: creds.refresh,
            ...(clientSecret ? { client_id: clientId, client_secret: clientSecret } : {}),
          }),
        });
...
function googleSelfHostNodes(cb: IntegrationPanelCallbacks, forFeature: string): HTMLElement[] {
  const p = para(`Create an OAuth Client ID (type "Web application") at console.cloud.google.com → APIs & Services → Credentials, enable the YouTube Data API v3 and Google Calendar API for the project, add this exact Authorized redirect URI, then paste the Client ID and Client Secret:`);
  ...
  const secretNote = para('Stored only in this browser, and sent only to Google and this site\u2019s own token-exchange endpoint — never logged.');
  const btn = connectBtn(`Connect Google for ${forFeature}`);
  btn.addEventListener('click', async () => {
    const id = inp.value.trim();
    const secret = secretInp.value.trim();
    if (!id || !secret) { cb.showToast('Enter both the Client ID and Client Secret'); return; }
