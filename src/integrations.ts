    ...
    const res = cfg.direct
      ? await fetch(cfg.tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            client_id: localStorage.getItem(cfg.clientIdKey) ?? "",
          }),
        })
      : await fetch(OAUTH_PROXY, {
          method: "POST",
...
function googleSelfHostNodes(
  ...
  const p = para(
    `Create an OAuth Client ID (type "Web application") at console.cloud.google.com → APIs & Services → Credentials, enable the YouTube Data API v3 and Google Calendar API for the project, add this exact Authorized redirect URI, then paste the Client ID:`,
  );
