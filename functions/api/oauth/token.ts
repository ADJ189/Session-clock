...
//
// Deliberately NOT supported: accepting a visitor-supplied client_id +
// client_secret pair to override the env-configured app on a per-
// request basis (Google's self-host form used to send one). That would
// mean storing an OAuth client secret in the browser's own localStorage
// — readable by any XSS, and, unlike a leaked access/refresh token,
// able to impersonate the whole app rather than just one user's
// session. Flagged by CodeQL as clear-text storage of sensitive data;
// removed rather than mitigated. See CONTRIBUTING.md and
// googleSelfHostNodes() in src/integrations.ts for what this means in
// practice: every proxied provider, Google included, only ever
// connects through this deployment's own configured app.
    ...
    return json(
      {
        ...
        message: `Set ${idKey} and ${secretKey} as Pages secrets to enable ${provider} OAuth.`,
      },
      501,
    );
  }
